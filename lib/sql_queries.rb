...
"SELECT projects.title, projects.funding_goal, (SUM(pledges.amount) - projects.funding_goal AS amount_over FROM projects INNER JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING amount_over >= 0"
end
